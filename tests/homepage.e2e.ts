...
test('renders the fantasy portal without broken artwork or overflow', async ({
	...
	await expect(page.locator('.threat-strip')).toHaveCount(0);
	await expect(page.locator('.manifesto-section')).toHaveCount(0);
	await expect(page.getByText('Preparation is power.')).toHaveCount(0);
	await expect(page.getByText('THE FIRST LAW')).toHaveCount(0);
	await expect(page.getByText('A realm for the relentless')).toHaveCount(0);
