...
The `migrate` service applies checked-in migrations, runs the idempotent seed with `ADMIN_EMAIL`, `ADMIN_USERNAME`, and `ADMIN_PASSWORD`, and exits before `web` starts. A normal redeploy therefore applies schema changes and keeps the admin account, demo users, and starter content current automatically.
